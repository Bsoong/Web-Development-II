...
let create = async function create(title: string, description: string, hoursEstimated: number, completed: boolean): Promise<object> {
...
let getAll = async function getAll(): Promise<object[]> {
    const tasksCollect = await tasks();
    const task = await tasksCollect.find({}).toArray();
    return task;
  }

let get = async function get(id: string): Promise<object> {
  const tasksCollect = await tasks();
  if(!id) {
...
let remove = async function remove(id: string):Promise<object> {
  const tasksCollect = await tasks();
  if(!id){
...
let update = async function update(id: string, updateTitle: string, updateDescription: string, updatehoursEstimated: number, updateCompleted: boolean):Promise<object> {
  const tasksCollect = await tasks();
...
let createComment = async function createComment(id: string, name: string, comment: string):Promise<object>  {
  const tasksCollect = await tasks();
...
let removeComment = async function removeComment(taskId: string, commentId:string):Promise<object>  {
...
let updateTitle = async function updateTitle(id: string, newTitle:string):Promise<object>  {
  const tasksCollect = await tasks();
...
let updateDescription = async function updateDescription(id: string, newDescription: string):Promise<object>  {
  const tasksCollect = await tasks();
  ...
  const updatedDescription: object = {
...
let updatehoursEstimated = async function updatehoursEstimated(id: string, newhoursEstimated: number):Promise<object>  {
  const tasksCollect = await tasks();
...
let updateCompleted = async function updateCompleted(id: string, newCompleted: boolean):Promise<object>  {
  const tasksCollect = await tasks();
